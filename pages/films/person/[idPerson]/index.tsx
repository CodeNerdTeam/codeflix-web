...
function index() {
  ...
  const [dataMovie, setDataMovie] = useState<PersonEntity>();
  ...
  useEffect(() => {
    getUser();
    getFilmsOfPerson(localStorage.getItem("idPerson"));
  }, []);
  ...
  const getFilmsOfPerson = async (id: string | null) => {
    const jwtString = sessionStorage.getItem("token");
    axios
      .get<PersonEntity>(`${baseUrl}/api/persons/${id}`, {
        headers: { Authorization: `Bearer ${jwtString}` },
      })
      .then((res) => {
