...
function index() {
  ...
  const [dataMovie, setDataMovie] = useState<GenreEntity>();
  ...
  useEffect(() => {
    getUser();
    getFilmsOfGenre(localStorage.getItem("idGenre"));
  }, []);
  ...
  const getFilmsOfGenre = async (id: string | null) => {
    const jwtString = sessionStorage.getItem("token");
    axios
      .get<GenreEntity>(`${baseUrl}/api/genres/${id}`, {
        headers: { Authorization: `Bearer ${jwtString}` },
      })
      .then((res) => {
